...
interface MidpointWithElevation {
  lat: number;
  lon: number;
  elevation?: number;
}
...
interface CourseWithInfo {
  ...
  midpoints: MidpointWithElevation[];
}

interface CompleteOutputData {
  base: {
    lat: number;
    lon: number;
  };
...
async function addAllInformation() {
  try {
    ...
    data.courses.forEach((course: any) => {
      course.midpoints.forEach((midpoint: any, index: number) => {
        allMidpoints.push({
          lat: midpoint.lat,
          lon: midpoint.lon,
          courseId: course.id,
    ...
    elevations.forEach((elevation, index) => {
      ...
      const course = data.courses.find((c: any) => c.id === midpointInfo.courseId);
    ...
    const finalData: CompleteOutputData = {
      base: data.base,
