...
import { circle12Points, divideLinePoints } from './geo';
...
async function generateRunningCourses() {
    try {
        ...
        endpoints.forEach((p, i) => console.log(`${i + 1}번 (${i * 30}°): lat=${p.lat.toFixed(6)}, lon=${p.lon.toFixed(6)}`));
        console.log('\n=== 각 점과의 3등분점 ===');
        const courses = endpoints.map((end, i) => {
            const midpoints = divideLinePoints(start, end);
            console.log(`${i + 1}번 (${i * 30}°):`);
            midpoints.forEach((mp, idx) => console.log(`  ${(idx + 1) * 25}%: lat=${mp.lat.toFixed(6)}, lon=${mp.lon.toFixed(6)}`));
            return {
                id: i + 1,
                angle: i * 30,
                start,
